  ...
  await Ticket.findOne({ _id: req.query.ticket_id }).then((ticket) => {
    ...
    if (req.query.role == "user") {
      ...
      Ticket.updateOne(
        ...
        { $set: { user_unreads: user_unreads === 0 } },
        function (err, db) {}
      );
    }
    if (req.query.role == "operator") {
      ...
      Ticket.updateOne(
        { _id: req.query.ticket_id },
        { $set: { op_unreads: operator_unreads === 0 } },
        function (err, db) {}
      );
    }
  });
...
exports.findUserTicket = (req, res) => {
  Ticket.find({ user_id: req.query.user_id })
    .then((ticket) => {
...
exports.answerTicket = (req, res) => {
  ...
  Ticket.findOne({ _id: req.query.ticket_id }).then((ticket) => {
    ...
    if (req.query.role == "user") {
      Ticket.updateOne(
        { _id: req.query.ticket_id },
        { $set: { op_unreads: operator_unreads + 1 } },
        function (err, db) {}
      );
    } else if (req.query.role == "operator") {
      Ticket.updateOne(
        { _id: req.query.ticket_id },
        { $set: { user_unreads: user_unreads + 1 } },
        function (err, db) {}
      );
    }
  });
